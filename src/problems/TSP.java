...
public class TSP implements Problem<int[]> {
    ...
    public int[] getInitState() {
        ...
        shuffleArray(tour);  // Start with a random tour
        return tour;
    }
    ...
    public int cost(int[] state) {
        ...
        for (int i = 0; i < numCities - 1; i++) {
            totalCost += distances[state[i]][state[i + 1]];
        }
        totalCost += distances[state[numCities - 1]][state[0]]; // Return to start
        return totalCost;
    }

    @Override
    public double goalCost() {
        return 0.0;  // No fixed goal cost, just minimize
    }

    @Override
    public int[] generateNewState(int[] currentState) {
        int[] newState = Arrays.copyOf(currentState, numCities);
        int i = rand.nextInt(numCities);
        int j = rand.nextInt(numCities);
        swap(newState, i, j); // Swap two cities to create a new tour
        return newState;
    }
    ...
    public void printState(int[] state) {
        ...
        System.out.println("Tour Cost: " + cost(state));
    }
    ...
    //The Sample record contains all four test cases; you can simply copy-paste this record into your TSP.java:
